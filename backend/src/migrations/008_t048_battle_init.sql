...
ALTER TABLE characters ADD COLUMN IF NOT EXISTS battle_id UUID REFERENCES battles(id) ON DELETE SET NULL;
CREATE INDEX IF NOT EXISTS idx_characters_battle_id ON characters(battle_id);

-- 2. characters 表加 deck_position（3v3 中棋子 0/1/2 位序，预留给未来）
ALTER TABLE characters ADD COLUMN IF NOT EXISTS deck_position INTEGER;
