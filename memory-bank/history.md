...
## 2026-08-08 - 任务：v0.1.3 部署核实 + 前端 T057 初始化

### Prompt
VPS 有改动，重新建立通讯；确认通讯链路正常；记录部署链路；开始前端框架搭建，T057 启动。

### 思考
- VPS 通讯链路：复用 SSH 凭证 + GitHub secrets；绕过本地代理直连 GitHub API 验证；health-check workflow 连续 5 次 success（VPS 在线）+ release/deploy workflow 状态确认
- 已 push tag v0.1.3（a2514ba，含 P2 后端改进1&2 + 前端骨架提交）-> Release run 31262817512 success -> Deploy run 31262952269 success 自动上线。确认 T-FOLLOW-7 部署问题已解决
- 前端：T057 初始化 Vue3 项目，骨架含类型定义（对齐 REST/WS 契约）、axios 拦截器（JWT 头 + 401 登出 + 信封剥离）、typed request helpers（httpGet/Post/Put/Delete 返回信封类型）、3 stores（auth 持久化 token / player 资源+仓库+棋子 / game WS 对战状态机）、路由守卫、登录/注册/主页（含离线收益弹窗）、5 占位视图（T064+ 待实现）

### 意外
1. 后端 register 只返回 User 不含 token，故 auth store register 后自动调 login 拿 token（原实现误取 res.data.token）
2. router 引用的 WorkshopView 等占位视图不存在导致 vite build 失败 -> 补 5 个占位视图
3. axios 拦截器返回类型与信封不匹配（TS 报 AxiosResponse vs 信封）-> http.ts 改 typed request helpers（httpGet/Post/Put/Delete 直接返回信封类型）

### 修复
- 新增 frontend/ 骨架（package.json / vite.config / tsconfig×2 / index.html / src/{main,App,env.d.ts,types,router,stores×3,services×2,views×6,assets}）
- backend/.dockerignore 排除 frontend
- 记录 v0.1.3 上线（P2 后端改进）
- memory-bank progress/history 同步

### 范围外
- T058-T063 随 T057 骨架就绪（路由/Pinia/登录注册/主页已可用），但未单独记录
- T064+ 前端界面（采集/仓库/制造/棋子/对战/匹配/结算）待开发
- 前端未上 CDN/独立构建；前端构建产物未部署
